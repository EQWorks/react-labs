...
const getHeader = (s) =>
  [s.charAt(0).toUpperCase(), s.slice(1).replace(/_/g, " ")].join("");
...
const useTableConfig = ({
  ...
  const [hidden, setHiddenCache] = cached({
    ...
    const _hidden = _cols
      .filter((c) => c.hidden)
      .map((c) => (typeof c.accessor === "string" ? c.accessor : c.id));
...
const Table = ({
  ...
  const { _cols, _data, hidden, setHiddenCache } = useTableConfig({
    data,
    hiddenColumns,
    children,
    columns,
  ...
  const {
  ...
  } = useTable(
    {
      ...
      initialState: {
        ...
        sortBy: useMemo(
          () => (Array.isArray(cachedSortBy) ? cachedSortBy : [cachedSortBy]),
          [cachedSortBy]
        ),
      },
    },
  ...
  return (
    <>
      ...
      {visibleColumns.length > 0 ? (
          ...
          <MUITable {...getTableProps(tableProps)}>
            <TableHead>
              {headerGroups.map((headerGroup, i) => (
                <TableRow
                  key={i}
                  {...headerGroup.getHeaderGroupProps(headerGroupProps)}
                >
            ...
            {(0 < rows.length && rows.length < data.length
        ...
        <Card>
          <CardContent>
            <Typography variant="body1">No visible columns</Typography>
          </CardContent>
        </Card>
      )}
    </>
...
const childrenColumnCheck = (props, _, componentName) => {
  if (props.children && props.columns) {
    return new Error(
      `Only one or none of 'children' or 'columns' is allowed in '${componentName}'`
    );
  }
};
